	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="main_table">
		...
		<c:choose>
			<c:when test="${not empty menuList}">
				...
				<a href="###" onclick="delmenu(${menu.menuId },true,${menu.isSys})">删除</a></td>
				</tr>
				</c:forEach>
			</c:when>
			<c:otherwise>
	...
	<script type="text/javascript">
		...
		function editmenu(menuId){
			var dg = new $.dialog({
				...
				xButton:true,
				resize:false,
				page:'menu/edit.do?menuId='+menuId
				});
    		dg.ShowDialog();
		}
		
		function delmenu(menuId,isParent,isSys){
			if(isSys==1){
				alert("系统菜单，不能删除！！！");
				return;
			}
			...
			if(flag){
				var url = "menu/del.do?menuId="+menuId;
		...
		function openClose(menuId,curObj,trIndex){
			...
			if(txt=="展开"){
				...
				var url = "menu/sub.do?menuId="+menuId;
				$.get(url,function(data){
					//alert(data);
					if(data.length>0){
						var html = "";
						$.each(data,function(i){
							...
							html += "<td><a href='###' onclick='editmenu("+this.menuId+")'>修改</a> | <a href='###' onclick='delmenu("+this.menuId+",false,"+this.isSys+")'>删除</a></td>";
							html += "</tr>";

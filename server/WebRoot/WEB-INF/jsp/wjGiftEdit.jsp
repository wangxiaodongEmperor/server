...
.info td {
	...
	function queryPic(){
		...
		if (object.val() != 0) {
			...
			$.ajax({
				type : 'post',
				url : '${path}/GiftPicUploadCtrl/queryPic.do',
				data : {
					giftid : object.val()
				},
				dataType : 'json',
				...
				success : function(result) {
					$.each(result, function(entryIndex, entity) {
						...
						$("#div_coverimgid").html("<div id='"+entity.fileid+1+"'><input id='imageid' type='hidden' name='imageid' value='"+entity.fileid+"'/></div>");
		...
		$.ajaxFileUpload({
			...
			success : function(json) {
				if (json.resultFlag == false) {
					...
					//提交fileid到block
					var ids = $("#div_coverimgid").html();
					$("#div_coverimgid").html("<div id='"+json.fileid+1+"'><input id='imageid' type='hidden' name='imageid' value='"+json.fileid+"'/></div>");
					$("#div_coverimgid").append(ids);
	...
	*/
	...
	<body onload="load()">
		<form action="update.do" name="userForm" id="userForm" target="result" method="post" onsubmit="return checkInfo();">
			...
			<input type="hidden" id="id" name = "id" value="${Gift4WJ.id }" />
			<input type="hidden" id="blockid" name="blockid" value="${Gift4WJ.blockid }" />
			<input type="hidden" id="hidden_isspecial" value="${Gift4WJ.isspecial }" />
			<input type="hidden" id="hidden_isnew" value="${Gift4WJ.isnew }" />
			<input type="hidden" id="hidden_giftsex" value="${Gift4WJ.giftsex }" />
			<input type="hidden" id="hidden_seltarget" value="${Gift4WJ.gifttarget }" />
			<input type="hidden" id="hidden_selfunction" value="${Gift4WJ.giftfunction }" />
			<input type="hidden" id="hidden_selpersonality" value="${Gift4WJ.giftpersonality }" />
			<input type="hidden" id="hidden_selpersonality" value="${Gift4WJ.giftpersonality }" />
				...
				<tr>
					<td align="left">
						<div
							...
							玩具标题：
							<input id="title" type="text" name="title" width="500"
								value="${Gift4WJ.title }" />
						</div>
					</td>
				</tr>
				<tr>
					<td align="left">
						<div
							style="display: block; padding-top: 10px; padding-left: 20px;">
							跳转链接：
							<input id="linkurl" type="text" name="linkurl" value="${Gift4WJ.linkurl }"/>
				...
				<tr>
					<td align="left">
						<div
							style="display: block; padding-top: 10px; padding-left: 20px;">
							<span>设置权重：</span>
							<input id="weight" type="text" name="weight" value=${Gift4WJ.weight } />
							<span style="color: red;">(默认写 1)</span>

							<span style="padding-left: 20px;">玩具编码：</span>
							<input id="giftcode" type="text" name="giftcode" value=${Gift4WJ.giftcode } />
				...
				<tr>
					<td align="left">
						<div style="display: block; padding-top: 10px; padding-left: 20px;">
							<span>输入现价：</span>
							<input id="currentprice" type="text" name="currentprice" value="${Gift4WJ.currentprice }" onkeyup="checkPrice(this)" onblur="chkLast(this)"/>

							<span style="padding-left: 20px;">输入原价：</span>
							<input id="originalprice" type="text" name="originalprice" value="${Gift4WJ.originalprice }" onkeyup="checkPrice(this)" onblur="chkLast(this)"/>
						
							<span style="padding-left: 20px;">输入折扣：</span>
							<input id="discount" type="text" name="discount" value="${Gift4WJ.discount }" onkeyup="checkPrice(this)" onblur="chkLast(this)"/>
						</div>
					</td>
				</tr>				
				<tr>
				...
				<tr>
					<td align="left">
						<div
							...
							<br />
							<textarea id ="detail"  rows="7" cols="102" name="detail" >${Gift4WJ.detail }</textarea>
						</div>
					</td>
				</tr>
			</table>
